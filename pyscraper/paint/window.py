...
def __check_page() -> None:
    try:
        ...
        exc_type, exc_obj, exc_tb = exc_info() # no error
        __add_log(f"{str(exception)} | {__check_page.__name__} | Line {exc_tb.tb_lineno}")
...
def __scrap_page(page: Page) -> None:
    try:
        ...
        exc_type, exc_obj, exc_tb = exc_info() # no error
        __add_log(f"{str(exception)} | {__scrap_page.__name__} | Line {exc_tb.tb_lineno}")
...
def __paint_page(tree: Tree, tag_page: str, page: Page) -> None:
    try:
        ...
        exc_type, exc_obj, exc_tb = exc_info() # no error
        __add_log(f"{str(exception)} | {__paint_page.__name__} | Line {exc_tb.tb_lineno}")
...
def __add_message(text: str, error: bool) -> None:
    ...
    __scrapt_error_label.configure(text=text, foreground=color)
...
def show():
    ...
    global __scrapt_error_label
    __scrapt_error_label = Label(master=scrap_entry_frame, font=(font_tabs, 11), background=color_indigo_dye)
    __scrapt_error_label.pack(side="top", fill="x", expand=True, pady=10)
    ...
    scrap_button.grid(row=4, column=0, pady=40)
